...
describe("BAP578", function () {
  ...
  describe("Agent Creation", function () {
    it("Should create an agent with free mint", async function () {
      ...
      const tx = await nfa.connect(addr1).createAgent(
        addr1.address,
        ethers.constants.AddressZero,
        "ipfs://metadata1",
        metadata
  ...
  describe("Agent Management", function () {
    ...
    it("Should only allow owner to manage agent", async function () {
      await expect(
        nfa.connect(addr2).setAgentStatus(1, false)
      ).to.be.revertedWith("Not token owner");
      
      await expect(
        nfa.connect(addr2).withdrawFromAgent(1, 100)
      ).to.be.revertedWith("Not token owner");
      
      await expect(
        nfa.connect(addr2).setLogicAddress(1, addr2.address)
      ).to.be.revertedWith("Not token owner");
    });
  });
  ...
  describe("Admin Functions", function () {
    ...
    it("Should only allow owner to call admin functions", async function () {
      await expect(
        nfa.connect(addr1).setPaused(true)
      ).to.be.revertedWith("Ownable: caller is not the owner");
      
      await expect(
        nfa.connect(addr1).setTreasury(addr2.address)
      ).to.be.revertedWith("Ownable: caller is not the owner");
